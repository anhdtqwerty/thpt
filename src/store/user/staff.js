...
export default {
  ...
  actions: {
    fetchStaffsCount ({ commit }, filters = []) {
      ...
      return axios.get(STAFF_COUNT_API, { params })
        .then(count => {
    ...
    fetchStaffs ({ commit }, query = '') {
      return axios.get(STAFF_API + query)
        .then(staffs => commit('setStaffs', staffs))
        .catch(e => alert.error(e))
    },
    fetchStaffsByTitle ({ dispatch }, keyword) {
      if (!keyword) return
      return dispatch('fetchStaffs', { params: { 'title_contains': keyword }, hideLoading: true })
    },
    fetchStaff ({ commit }, staffId) {
      return axios.get(STAFF_API + staffId)
    ...
    updateStaff ({ commit, state }, { id, ...staff }) {
      return axios.put(STAFF_API + id, staff)
        .then(staff => {
          commit('setStaff', staff)
          commit('receiveStaff', staff)
    ...
    async removeStaff ({ commit }, staff) {
      ...
      await axios.delete(STAFF_API + staff.id)
        .then(staff => {
