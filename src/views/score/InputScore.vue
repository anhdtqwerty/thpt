                 ...
                 <v-row>
                    <v-col cols="12">
                      <v-row class="basic-filter" v-if="filterMode === 'normal'">
                        <v-col cols="12" md="4">
                          <autocomplete-class placeholder="Chọn lớp" filled dense @change="filterInputs.classObj = $event" />
                        </v-col>
                        <v-col cols="12" md="3">
                          <autocomplete-subject return-object placeholder="Chọn môn học" filled dense @change="filterInputs.subjectObj = $event" />
                        </v-col>
                        <v-col cols="12" md="3">
                            <autocomplete-factor placeholder="Chọn đầu điểm" filled dense @change="filterInputs.factorObj = $event" />
                        </v-col>
                        <v-col cols="12" md="2">
                      ...
                      <v-row class="advance-filter" v-if="filterMode === 'advanced'">
                        <v-col cols="12" md="4">
                          <autocomplete-semeter placeholder="Chọn học kỳ" filled dense @change="filterInputs.semesterObj = $event" />
                        </v-col>
                        <v-col cols="12" md="4">
                          <autocomplete-class placeholder="Chọn lớp" filled dense @change="filterInputs.classObj = $event" />
                        </v-col>
            ...
            <template v-if="filterInputs.subjectObj">
                  ...
                  >
                    <template v-slot:item.mark="{ item }">
                      <input v-for="mark in item.marks" :key="mark.id" class="custom-input" type="text" :value="mark.value" @input="event => onInputMark(item.studentId, mark.id, event.target.value)">
                    </template>
                  </v-data-table>
                </template>
                <template v-else>
    ...
    marks (data) {
      this.items = this.groupBy('studentId')(
        this.generateDataTable(
          this.setOrderForMark(Object.values(data)
          )
        )
      )
    }
      ...
      return marks => {
        return this.findAndUpdate(marks)('studentId')(studentId)(item => {
          return {
            ...item,
            tag: 'updated',
            marks: this.findAndUpdate(item.marks)('id')(markId)(m => ({ ...m, tag: 'updated', value: mark }))
          }
        })
      }
    },
