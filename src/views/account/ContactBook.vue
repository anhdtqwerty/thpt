<template>
  <div class="pa-4">
    <ConfigContactDialog :state="configDialog" :student="selectedStudent" @close="configDialog = false" />
      ...
      >
        ...
        <template v-slot:[`item.action`]="{ item }">
          <v-menu left offset-x>
            ...
            <v-list>
              ...
              <v-list-item @click="unlockContact(item)" v-if="item.contact && item.contact.isLocked">
        ...
        <template v-slot:[`item.status`]="{ item }">
          ...
          <v-chip
            color="cancel"
            style="width: 105px"
            v-else-if="!item.contact || !item.contact.phones || !item.contact.phones.length === 0"
            ><span class="white--text">Chưa cài đặt</span></v-chip
          >
          <v-chip color="success" style="width: 105px" class="d-flex justify-center" v-else>Hoạt động</v-chip>
        </template>
        <template v-slot:[`item.classes`]="{ item }">
          <v-btn style="letter-spacing: 0.2px !important" class="d-flex justify-start px-0" color="primary" plain>{{
            item.classes | getClasses
          }}</v-btn>
        </template>
...
export default {
  ...
  filters: {
    ...
    getSenderMethod(item) {
      ...
      else if (item.contact && item.contact.isSms && item.contact.isApp) return 'APP và SMS'
      return 'SMS'
    },
    getPhone(item) {
      if (!item.contact || !item.contact.phones) return ''
      else if (typeof item.contact.phones === 'string') return item.contact.phones
